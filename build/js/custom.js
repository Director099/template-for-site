	...
	+function ($) {
	  ...
	  function clearMenus(e) {
	    ...
	    $(toggle).each(function () {
	      ...
	      if (e && e.type == 'click' && /input|textarea/i.test(e.target.tagName) && $.contains($parent[0], e.target)) return; // Выходим, если клик пришелся на элемент внутри .dropdown__menu

	      if (e && e.type == 'click' && /dropdown__menu/i.test(e.toElement.offsetParent.className)) return; // $parent.trigger(e = $.Event('hide.bs.dropdown', relatedTarget))
	  ...
	  Dropdown.prototype.toggle = function (e) {
	    var $this = $(this); // if ($this.is('.disabled, :disabled')) return
	    ...
	    if (!isActive) {
	      ...
	      .toggleClass('dropdown--open') // .trigger($.Event('shown.bs.dropdown', relatedTarget))
	  ...
	  Dropdown.prototype.keydown = function (e) {
	    ...
	    var $this = $(this);
	    e.preventDefault();
	    e.stopPropagation(); // if ($this.is('.disabled, :disabled')) return
	  ...
	  function Plugin(option) {
	    return this.each(function () {
	      var $this = $(this); // var data  = $this.data('bs.dropdown')

	      var data = $this.data('nth.dropdown'); // if (!data) $this.data('bs.dropdown', (data = new Dropdown(this)))
	  ...
	  .on('click.nth.dropdown.data-api', clearMenus).on('click.nth.dropdown.data-api', '.dropdown form', function (e) {
	    e.stopPropagation();
	  }).on('click.nth.dropdown.data-api', toggle, Dropdown.prototype.toggle).on('keydown.nth.dropdown.data-api', toggle, Dropdown.prototype.keydown).on('keydown.nth.dropdown.data-api', '.dropdown__menu', Dropdown.prototype.keydown);
	}(jQuery);
